 ...
 *							ajout gestion login et variable
 *d'environnement 10/04/07 (Samy Pessé) : la classe System gere maintenant une
 *liste chainé des utilisateur (User)
 **/
...
int System::login(User *us, char *pass) {
  ...
  if (us->getPassword() != NULL) { // si il ya un password
    ...
    if (pass == NULL) // si on a passé un code
      return PARAM_NULL;
    //
    if (strncmp(pass, us->getPassword(),
                strlen(us->getPassword()))) // test password
