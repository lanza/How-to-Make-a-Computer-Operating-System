...
void Filesystem::init() {
  ...
  dev = root->createChild("dev",
                          TYPE_DIRECTORY); // dossier contenant les peripherique
  root->createChild("proc",
                    TYPE_DIRECTORY); // dossier contenant les processus tournant
  root->createChild(
      "mnt",
      TYPE_DIRECTORY); // dossier contenant les points de montages des disques
  File *sysd = root->createChild(
      "sys", TYPE_DIRECTORY); // dossier contenant toutes les infos du systemes
  var = sysd->createChild(
      "env",
      TYPE_DIRECTORY); // dossier contenant toutes les variables d'environnement
  sysd->createChild("usr",
                    TYPE_DIRECTORY); // dossier contenant tous les utilisateurs
  sysd->createChild(
      "mods", TYPE_DIRECTORY); // dossier contenant tous les modules disponiles
  sysd->createChild(
      "sockets", TYPE_DIRECTORY); // dossier contenant tous les sockets actuels
}
...
File *Filesystem::getRoot() { return root; }
...
File *Filesystem::pivot_root(File *targetdir) {
  if (targetdir == NULL)
    ...
    s8 i, ii = 0;
    File *tempChild =
        mainChild->getPrec(); // Est que File doit être initialisé ? ou pas ?
    do {
      if (tempChild == NULL) {
        i = 0;
      } else {
