...
u32 Process::open(u32 flag) { return RETURN_OK; }

u32 Process::close() { return RETURN_OK; }

Process *Process::getPParent() { return pparent; }

void Process::setPParent(Process *p) { pparent = p; }
...
void Process::setPNext(Process *p) { pnext = p; }

process_st *Process::getPInfo() { return &info; }

Process *Process::getPNext() { return pnext; }
...
u32 Process::create(char *file, int argc, char **argv) {
  ...
  if (pparent != NULL) {
    memcpy((char *)&openfp[0], (char *)pparent->getFileInfo(0),
           sizeof(openfile));
    memcpy((char *)&openfp[1], (char *)pparent->getFileInfo(1),
           sizeof(openfile));
    memcpy((char *)&openfp[2], (char *)pparent->getFileInfo(2),
           sizeof(openfile));
    addFile(this, 0);
  } else {
...
File *Process::getCurrentDir() { return cdir; }

void Process::setCurrentDir(File *f) { cdir = f; }

void Process::setState(u8 st) { state = st; }

u8 Process::getState() { return state; }

void Process::setPid(u32 st) { pid = st; }

u32 Process::getPid() { return pid; }

void Process::sendSignal(int sig) { set_signal(&(info.signal), sig); }
